...
function parseMeasure(group, numberOfBars) {
    var attributes, timestamp, returnValue, groupValue;

    groupValue = group;

    if (numberOfBars && numberOfBars.length % 2 === 0) {
        timestamp = groupValue.substr(0, groupValue.indexOf('|'));
        groupValue = groupValue.substr(groupValue.indexOf('|') + 1);
    }

    attributes = groupValue.split('|');
    ...
    if (
        ...
        throw new errors.ParseError('Parsing group:' + groupValue);
    } else {
...
function command(payload) {
    ...
    results.deviceId = deviceData[0];
    results.command = deviceData[1];

    dataSection = fields.splice(1);

    if (dataSection.length === 1 && dataSection[0].indexOf('=') < 0) {
        results.value = dataSection[0];
    } else {
        results.params = dataSection.reduce(addAttribute('='), {});
    }
...
function parseGroup(group) {
    var numberOfBars, groupValueParse;

    groupValueParse = group;

    if (groupValueParse[0] === '|') {
        groupValueParse = groupValueParse.substr(1);
    }

    numberOfBars = groupValueParse.match(/\|/g);

    if (groupValueParse.indexOf('@') > 0) {
        return parseCommand(groupValueParse);
    } else {
        return parseMeasure(groupValueParse, numberOfBars);
    }
}
 ...
 */
function createCommandPayload(device, commandPayLoad, attributes) {
    function addAttributes(current, key) {
        return current + '|' + key + '=' + attributes[key];
    }

    if (typeof attributes === 'object') {
        return Object.keys(attributes).reduce(addAttributes, device.id + '@' + commandPayLoad);
    } else {
        return device.id + '@' + commandPayLoad + '|' + attributes;
    }
}
