...
function parseGroup(group) {
    var numberOfBars, groupValue;

    groupValue = group;

    if (groupValue[0] === '|') {
        groupValue = groupValue.substr(1);
    }

    numberOfBars = groupValue.match(/\|/g);

    if (groupValue.indexOf('@') > 0) {
        return parseCommand(groupValue);
    } else {
        return parseMeasure(groupValue, numberOfBars);
    }
}
